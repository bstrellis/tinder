// BEGINNING OF CODE TO CHANGE PROFILES
var nextProfileIndex = 0;

var zoomedIn = false;

// FUNCTION TO SWITCH TO NEXT PROFILE. ACTIVATED BY CLICKING LIKE/DISLIKE BUTTON
var setProfile = function() {
  // retrieve profile we want to show
  var profile = profiles[nextProfileIndex];
  nextProfileIndex = nextProfileIndex + 1;

  var everythingButChoices = document.createElement('div');
  everythingButChoices.className = 'everythingButChoices';

  var headerElement = document.createElement('div');
  headerElement.className = 'headerElement';

  var settingsElement = document.createElement('img');
  settingsElement.className = 'settingsElement headerIcon';
  settingsElement.src = 'iconmonstr-gear-1-240.png';

  var headerTextElement = document.createElement('span');
  headerTextElement.className = 'headerTextElement';
  headerTextElement.innerText = 'tinder';

  var chatElement = document.createElement('img');
  chatElement.className = 'chatElement headerIcon';
  chatElement.src = 'iconmonstr-speech-bubble-23-240.png';

  // nameAndAgeElement.appendChild(nameElement);
  headerElement.appendChild(settingsElement);
  headerElement.appendChild(headerTextElement);
  headerElement.appendChild(chatElement);

  var pictureFrameElement = document.createElement('div');
  pictureFrameElement.className = 'pictureFrame';
  pictureFrameElement.src = profile.pictureUrls[0];

  var pictureElement = document.createElement('img');
  pictureElement.className = 'first-pic';
  pictureElement.src = profile.pictureUrls[0];

  var nameAndAgeElement = document.createElement('div');
  nameAndAgeElement.className = 'nameAndAgeElement';

  var nameElement = document.createElement('span');
  nameElement.className = 'name';
  nameElement.innerText = profile.name;

  var commaAndSpaceElement = document.createElement('span');
  commaAndSpaceElement.className = 'comma_and_space';
  commaAndSpaceElement.innerText = ', ';

  var ageElement = document.createElement('span');
  ageElement.className = 'age';
  ageElement.innerText = profile.age;

  nameAndAgeElement.appendChild(nameElement);
  nameAndAgeElement.appendChild(commaAndSpaceElement);
  nameAndAgeElement.appendChild(ageElement);

  var collegeElement = document.createElement('span');
  collegeElement.className = 'college';
  collegeElement.innerText = profile.college;

  pictureFrameElement.appendChild(pictureElement);
  pictureFrameElement.appendChild(nameAndAgeElement);
  pictureFrameElement.appendChild(collegeElement);


  var undersheetOneElement = document.createElement('div');
  undersheetOneElement.className = 'undersheetOneElement';

  var undersheetTwoElement = document.createElement('div');
  undersheetTwoElement.className = 'undersheetTwoElement';

  everythingButChoices.appendChild(headerElement);
  everythingButChoices.appendChild(pictureFrameElement);
  everythingButChoices.appendChild(undersheetOneElement);
  everythingButChoices.appendChild(undersheetTwoElement);
  document.querySelector('.screen').appendChild(everythingButChoices);

// Choices from initial profile view
  var choiceBarElement = document.createElement('div');
  choiceBarElement.className = 'choiceBarElement';

  var goBackButtonElement = document.createElement('button');
  goBackButtonElement.className = 'goBackButtonElement small_icon';

  var likeButtonElement = document.createElement('button');
  likeButtonElement.className = 'likeButtonElement large_icon';

  var dislikeButtonElement = document.createElement('button');
  dislikeButtonElement.className = 'dislikeButtonElement large_icon';

  var superLikeButtonElement = document.createElement('button');
  superLikeButtonElement.className = 'superLikeButtonElement small_icon';

  choiceBarElement.appendChild(goBackButtonElement);
  choiceBarElement.appendChild(likeButtonElement);
  choiceBarElement.appendChild(dislikeButtonElement);
  choiceBarElement.appendChild(superLikeButtonElement);
  document.querySelector('.screen').appendChild(choiceBarElement);


  zoomedIn = false;

  likeButtonElement.addEventListener('click', moveToNextProfile);
  dislikeButtonElement.addEventListener('click', moveToNextProfile);


  pictureElement.addEventListener('click', zoomIntoProfile);
};

// FUNCTION TO REMOVE HTML ON PAGE BEFORE RUNNING setProfile
var removeProfile = function() {
  document.querySelector('.first-pic').parentNode.removeChild(document.querySelector('.first-pic'));
  document.querySelector('.name').parentNode.removeChild(document.querySelector('.name'));
  document.querySelector('.age').parentNode.removeChild(document.querySelector('.age'));
  document.querySelector('.college').parentNode.removeChild(document.querySelector('.college'));
  // document.querySelector('.everythingButChoices').removeChild(document.querySelector('.pictureFrame'));
  // document.querySelector('.everythingButChoices').removeChild(document.querySelector('.undersheetOneElement'));
  // document.querySelector('.everythingButChoices').removeChild(document.querySelector('.undersheetTwoElement'));
  // document.querySelector('.everythingButChoices').removeChild(document.querySelector('.headerElement'));
  document.querySelector('.screen').removeChild(document.querySelector('.everythingButChoices'));
  document.querySelector('.screen').removeChild(document.querySelector('.choiceBarElement'));
};

// moveToNextProfile checks to see whether view is initial or zoomed
// then removes html and sets new initial profile. If there are no new profiles,
// it displays "There are no more profiles available!"
var moveToNextProfile = function () {
  if (nextProfileIndex <= profiles.length - 1) {
    if (zoomedIn === true) {
      switchZoomProfile();
      zoomedIn = false;
    } else {
      switchProfile();
    }
  } else {
    alert('Sorry, mi amigo, but there are no more candidatos elegibles!');
  }

};

// FUNCTION TO CONTAIN remove AND setProfile FUNCTIONS SO THEY CAN BE CALLED SIMULTANEOUSLY
var switchProfile = function() {
  removeProfile();
  setProfile();
};

// FUNCTION TO REMOVE ELEMENTS IN ZOOM VIEW
var removeZoomProfile = function () {
  document.querySelector('.window').parentNode.removeChild(document.querySelector('.window'));
  document.querySelector('.data').parentNode.removeChild(document.querySelector('.data'));
  document.querySelector('.collegeEl').parentNode.removeChild(document.querySelector('.collegeEl'));
  document.querySelector('.distanceEl').parentNode.removeChild(document.querySelector('.distanceEl'));
  document.querySelector('.bioEl').parentNode.removeChild(document.querySelector('.bioEl'));
  document.querySelector('.jobEl').parentNode.removeChild(document.querySelector('.jobEl'));
};

// FUNCTION TO CONTAIN removeZoomProfile and setProfile SO PROFILES CAN BE SWITCHED FROM ZOOM VIEW
var switchZoomProfile = function () {
  removeZoomProfile();
  setProfile();
};

var profiles = [
  {
    name: 'Dagny',
    age: 26,
    college: 'UC Santa Cruz',
    distance: '25 miles away',
    bio: 'I like anal sex, sloppy blowjobs, and multiple orgasms. In my free time, I love anal sex, sloppy blowjobs, and orgasms.  Most Friday nights involve anal sex, sloppy blowjobs, and multiple orgasms. Truly, there is nothing more important to me than anal sex, sloppy blowjobs, and multiple orgasms.',
    job: 'Musician',
    pictureUrls: [
      'data:image/jpeg;base64,',
      'http://static1.squarespace.com/static/543e424ce4b0c9fc7fdce26d/t/561d3b5be4b001f1ad428ad2/1444756316891/',
      'data:image/jpeg;base64,',
    ]
  },
  {
    name: 'Oprah',
    age: 22,
    college: 'UC Berkeley',
    distance: '3 miles away',
    bio: 'I am worth a lot of Money.  A Lot of Money, actually.  You would not even believe how much money I have.',
    job: 'Television Star',
    pictureUrls: [
      'http://static.oprah.com/images/o2/201404/oprah-tour-bio-1-949x534.jpg',
      'http://hollywoodpsychicinsider.com/wp-content/uploads/2013/12/oprahwinfrey_2005.jpg',
      'http://d39ya49a1fwv14.cloudfront.net/wp-content/uploads/2014/03/oprah-winfrey-167024_w1000.jpg',
      'http://www.realsuccess.net/wp-content/uploads/2015/10/oprah-winfrey-top-tv-show-wallpaper-body-8ed700a6d0de777200111377efb82f53-big-21008.jpg',
      'http://img.timeinc.net/time/time100/2007/images/oprah_winfrey.jpg'
    ]
  },
  {
    name: 'Alicia',
    age: 87,
    college: 'UC Los Angeles',
    distance: '10 miles away',
    bio: 'Send dick pics.',
    job: 'Singer/songwriter',
    pictureUrls: [
      'http://i.huffpost.com/gen/1782331/thumbs/o-ALICIA-KEYS-GIVENCHY-570.jpg',
      'http://speakerdata.s3.amazonaws.com/photo/image/781014/alicia_keys.jpg'
    ]
  }
];
// END OF CODE TO CHANGE PROFILES

// CALL FIRST PROFILE IN ARRAY WHEN WEBSITE LOADS
setProfile();

// BEGINNING OF CODE TO ZOOM INTO A PROFILE
var zoomIntoProfile = function() {
  removeProfile();

  // createElements
  // create, style and append each element
  var pictureWindowEl = document.createElement('div');
  pictureWindowEl.className = 'window';
  document.querySelector('.screen').insertBefore(pictureWindowEl, document.querySelector('.choices'));

  // load each img from profile.pictureUrls to pictureWindowEl
  var profile = profiles[nextProfileIndex - 1];

  for (var u = 0; u < profile.pictureUrls.length; u++) {
    var pictureEl = new Image(500, 500);
    pictureEl.src = profile.pictureUrls[u];
    pictureWindowEl.appendChild(pictureEl);
  }

  // var nameElement = document.createElement('span');
  // nameElement.className = 'name';
  // nameElement.innerText = profile.name;

  var personDataEl = document.createElement('div');
  personDataEl.className = 'data underPicture ';
  document.querySelector('.screen').insertBefore(personDataEl, document.querySelector('.choices'));

  var nameEl = document.createElement('span');
  nameEl.className = 'nameEl';
  nameEl.innerText = profile.name;
  document.querySelector('.screen').insertBefore(nameEl, document.querySelector('.choices'));

  var commaEl = document.createElement('span');
  commaEl.className = 'commaEl';
  commaEl.innerText = ',';
  document.querySelector('.screen').insertBefore(commaEl, document.querySelector('.choices'));

  var ageEl = document.createElement('span');
  ageEl.className = 'ageEl';
  ageEl.innerText = profile.age;
  document.querySelector('.screen').insertBefore(ageEl, document.querySelector('.choices'));

  personDataEl.appendChild(nameEl);
  personDataEl.appendChild(commaEl);
  personDataEl.appendChild(ageEl);

  var jobEl = document.createElement('span');
  jobEl.className = 'underPicture jobEl smallGrayText';
  jobEl.innerText = profile.job;
  document.querySelector('.screen').insertBefore(jobEl, document.querySelector('.choices'));

  var collegeEl = document.createElement('span');
  collegeEl.className = 'underPicture collegeEl smallGrayText';
  collegeEl.innerText = profile.college;
  document.querySelector('.screen').insertBefore(collegeEl, document.querySelector('.choices'));

  var distanceEl = document.createElement('div');
  distanceEl.className = 'underPicture distanceEl smallGrayText';
  distanceEl.innerText = profile.distance;
  document.querySelector('.screen').insertBefore(distanceEl, document.querySelector('.choices'));

  var bioEl = document.createElement('div');
  bioEl.className = 'underPicture bioEl smallGrayText';
  bioEl.innerText = profile.bio;
  document.querySelector('.screen').insertBefore(bioEl, document.querySelector('.choices'));

  zoomedIn = true;
};
